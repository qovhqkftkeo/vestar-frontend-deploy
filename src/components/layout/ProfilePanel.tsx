...
export function ProfilePanel({ open, onClose }: ProfilePanelProps) {
  ...
  const handleMintMockUsdt = async () => {
    ...
    try {
      ...
      const hash = await mintMockUsdt(
        walletClient,
        address,
        MOCK_USDT_MINT_AMOUNT,
      );
      ...
      addToast({
        ...
        message:
          lang === "ko"
            ? "1,000 MockUSDT가 지급되었습니다."
            : "1,000 MockUSDT minted.",
      });
    } catch (error) {
