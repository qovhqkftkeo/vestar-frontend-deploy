...
import {
  encryptBallotWithPublicKey,
  randomNonceHex,
} from "../../utils/privateBallot";
import {
...
function getInsufficientPaidVoteBalanceMessage(
...
function trimPreparedVoteSubmissionCache(
  cache: Map<string, PreparedVoteSubmission>,
) {
...
async function prepareVoteSubmission(
  ...
  if (
    ...
    quotedPayment = await quoteElectionPayment(
      vote.electionAddress as Address,
      1,
    );
    if (quotedPayment > 0n) {
...
export function useVoteSubmit(
  ...
  const preparedSubmissionCacheRef = useRef<
    Map<string, PreparedVoteSubmission>
  >(new Map());
  const preparedSubmissionPromiseRef = useRef<
    Map<string, Promise<PreparedVoteSubmission>>
  >(new Map());
  ...
  const ensurePreparedSubmission = useCallback(
    async (
      ...
      const key = buildVoteSubmitPreparationKey(
        vote,
        candidateKeys,
        voterAddress,
      );
      ...
      if (
        cached &&
        Date.now() - cached.createdAt < VOTE_SUBMIT_PREFLIGHT_TTL_MS
      ) {
        return cached;
      }
      ...
      const nextPromise = prepareVoteSubmission(
        vote,
        candidateKeys,
        voterAddress,
      )
  ...
  useEffect(() => {
    if (!preflightKey || !preflightVote || !walletClient?.account?.address)
      return;
    ...
    async (vote: VoteDetailData | null, candidateKeys: string[]) => {
      ...
      try {
        ...
        if (!prepared.canSubmit) {
          ...
          const currentTierId =
            vote.minKarmaTier > 0
              ? await getKarmaTier(walletClient.account.address).catch(
                  () => undefined,
                )
              : undefined;
        ...
        if (
          ...
          if (quotedPayment > 0n) {
            ...
            if (balance < quotedPayment) {
              throw new Error(
                getInsufficientPaidVoteBalanceMessage(quotedPayment, lang),
              );
            }
          ...
          vote.visibilityMode === "PRIVATE"
            ? (() => {
                if (!prepared.encryptedBallot) {
                  throw new Error(
                    "Private election payload could not be prepared.",
                  );
                }
