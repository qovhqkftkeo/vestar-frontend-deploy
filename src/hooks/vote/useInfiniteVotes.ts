...
function getSubmissionCount(
  election: Pick<ApiElection, "resultSummary">,
): number {
  return election.resultSummary?.totalSubmissions ?? 0;
}

function compareByNewest(
  left: Pick<ApiElection, "id">,
  right: Pick<ApiElection, "id">,
): number {
  return Number(right.id) - Number(left.id);
}

function compareBySubmissionCount(
  left: ApiElection,
  right: ApiElection,
): number {
  const bySubmissionCount =
    getSubmissionCount(right) - getSubmissionCount(left);
...
export function useInfiniteVotes(
  ...
  useEffect(() => {
    ...
    const cachedItems = getViewCache<VoteListItem[]>(
      cacheKey,
      VOTE_LIST_CACHE_TTL_MS,
    );
      ...
      const loadAllVotes = async () => {
        ...
        try {
          const priorityRaw = dedupeElectionsById(
            (
              await Promise.all(
                ACTIVE_FIRST_ONCHAIN_STATES.map((state) =>
                  fetchElections({ onchainState: state }),
                ),
              )
            ).flat(),
          );
          ...
          const priorityItems = toVoteListItems(
            sortElections(priorityElections, filter),
          );
          ...
          if (cachedItems) {
            ...
            const mockFiltered =
              optimisticItems.length > 0
                ? optimisticItems
                : applyMockFilter(VOTE_ITEMS, filter);
        ...
        try {
          const endedRaw = dedupeElectionsById(
            (
              await Promise.all(
                ENDED_ONCHAIN_STATES.map((state) =>
                  fetchElections({ onchainState: state }),
                ),
              )
            ).flat(),
          ...
          const nextItems = toVoteListItems(
            sortElections(
              dedupeElectionsById([...priorityElections, ...endedElections]),
              filter,
            ),
          );
...
function applyMockFilter(
  ...
  switch (filter) {
    ...
    case "hot":
      return [...items]
        .filter(
          (vote) => vote.badge === "live" && parseVoteCount(vote.count) >= 1,
        )
        .sort(
          (left, right) =>
            parseVoteCount(right.count) - parseVoteCount(left.count),
        );
    case "new":
