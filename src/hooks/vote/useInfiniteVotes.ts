...
async function hydrateElections(elections: ApiElection[]) {
  return Promise.all(
    elections.map(async (election) => {
      ...
      const optimisticElection = applyOptimisticParticipantCountToElection(hydratedElection)
...
export function useInfiniteVotes(
  ...
  useEffect(() => {
    ...
    const optimisticItems = sortElections(mergeOptimisticElections([], { onchainState }), filter)
    ...
    if (filter === 'all') {
      ...
      const loadAllVotes = async () => {
        ...
        try {
          ...
          if (cancelled) return
          await waitForHomeDataFallback(requestStartedAt)
          if (cancelled) return
          if (!cachedItems && optimisticItems.length === 0) {
            recoveryTriggered = await recoverFromStaleHomeBundle()
            if (cancelled || recoveryTriggered) {
              return
            }
          }
          if (cachedItems) {
      ...
      fetchElections({ onchainState })
        ...
        .finally(() => {
          if (!cancelled && !recoveryTriggered) setIsLoading(false)
        })
    }
