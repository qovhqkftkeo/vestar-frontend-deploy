  ...
  return (
    <div className="fixed inset-0 z-[500] flex items-center justify-center px-5">
      ...
      <div className="relative z-[501] w-full max-w-[340px] bg-white rounded-3xl p-6 shadow-[0_20px_60px_rgba(0,0,0,0.30)] animate-[slideUp_0.22s_ease-out]">
        ...
        {/* Target */}
        <p className="text-[14px] text-[#090A0B] text-center mb-3 font-semibold">
          {target}
        </p>
...
export function VoteDetailPage() {
  ...
  const isWrongNetwork =
    !!vote?.electionAddress && chainId !== vestarStatusTestnetChain.id;
  ...
  useEffect(() => {
    ...
    setConfig({
      ...
      onShare: () => {
        if (navigator.share) {
          navigator
            .share({ title: vote.title, url: window.location.href })
            .catch(() => {});
        }
      },
    });
      ...
      >
        {isWrongNetwork ? (
          <button
            type="button"
            onClick={() =>
              switchChainAsync({ chainId: vestarStatusTestnetChain.id })
            }
