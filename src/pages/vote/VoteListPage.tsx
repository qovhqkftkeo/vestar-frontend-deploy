...
function VoteItem({
  ...
  const { t } = useLanguage();
  const badgeLabel =
    item.badge === "end" ? t("badge_end") : BADGE_LABEL[item.badge];
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1 mb-1">
          <span className="text-[11px] text-[#707070] font-mono truncate">
            {item.org}
          </span>
          {item.verified && (
            <img
        ...
        <div className="text-[12px] text-[#707070]">
          <strong className="text-[#7140FF] font-mono">{item.count}</strong>{" "}
          {t("vl_participating")}
        </div>
      </div>
      ...
      <div className="bg-[#FFFFFF]">
        ...
        <div className="px-5 pb-1 flex gap-3 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {isHotLoading
            ...
            : hotVotes.map((v) => (
                <HotVoteCard key={v.id} vote={v} onNavigate={handleNavigate} />
              ))}
        </div>
